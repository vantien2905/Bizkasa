//
//  UIViewPlaceholder+Extension.swift
//  Vebrary
//
...
open class Loader {
    static func addLoaderToViews(_ view : [UIView]) {
        CATransaction.begin()
//        for child in view.subviews
        view.forEach { $0.ld_addLoader() }

        CATransaction.commit()
    }

    static func removeLoaderFromViews(_ view: [UIView]) {
        CATransaction.begin()
        view.forEach { $0.ld_removeLoader()}
        CATransaction.commit()
    }
